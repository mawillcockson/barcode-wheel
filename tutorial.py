...
drawing.add(
    Rect(
        insert=(0, 0), size=(width, height), fill="none", stroke="black", stroke_width=1
    )
)

# Highlight points
points = [(0, 0), (0, width), (width, 0), (0, height), (width, height)]
...
def draw_slice(center, radius, start_angle, stop_angle, **kwargs):
    ...
    p_a.push_arc(
        target=(cos(angle) * radius + center[0], sin(angle) * radius + center[1]),
        rotation=0,
        r=radius,
    ...
    p_a.rotate(
        angle=(min([start_angle, stop_angle]) + (stop_angle - start_angle) / 2.0),
        center=center,
    )
